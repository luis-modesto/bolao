...
class Apostador extends Usuario{
	...
	*/
	function __construct($cpf, $nome, $senha, $boloesCriados, $boloesParticipa, $boloesEncerrados, $saldo, $apostas, $convites, $solicitacoes){
		...
		$users = (DataGetter::getInstance())->getData('usuarios');
	...
	function efetuarLogin($cpf, $senha) {
		...
		for ($i = 0; $i<count($users); $i++){
			if ($cpf==$users[$i][0] && $senha==$users[$i][1]){
				...
				$boloes = (DataGetter::getInstance())->getData('bolao');
				for ($j = 0; $j<count($boloes); $j++){
					if ($boloes[$j][1]==$cpf){

						$jogos = array();
						$jogosBolao = (DataGetter::getInstance())->getData('jogos_' . $boloes[$j][0]);
				...
				$boloes = (DataGetter::getInstance())->getData('boloes_' . $cpf);
				for ($j = 0; $j<count($boloes); $j++){
					$b = new Bolao($boloes[$j][0], $boloes[$j][2], $boloes[$j][3], $boloes[$j][4], $jogos,$boloes[$j][1], explode(',', $boloes[$j][5]), explode(',', $boloes[$j][6]), intval($boloes[$j][7]), intval($boloes[$j][8]), intval($boloes[$j][9]), intval($boloes[$j][10]));
					if ($boloes[$j][0]=="ativo"){
				...
				$matrizApostas = (DataGetter::getInstance())->getData('apostas_' . $cpf);
				...
				$notificacoes = (DataGetter::getInstance())->getData('notificacoes_' . $cpf);
	...
	*/
	function editarAposta($aposta, $novoPlacar) {
		if($aposta->isEditavel == true){
			...
			$matrizApostas = (DataGetter::getInstance())->getData('apostas_' . $this->cpf);
	...
	*/
	function responderConviteBolao($convite, $resposta) {
		$notificacoes = (DataGetter::getInstance())->getData('notificacoes_' . $this->cpf);
		$novasNot = array();
		...
		if($resposta == true){
			...
			(DataGetter::getInstance())->appendData('boloes_' . $this->cpf, 'ativo;' . $convite->bolao); //adiciona a boloes que participa
		}
	...
	*/
	function responderSolicitacao($solicitacao, $resposta) {
		...
		if($resposta == true){
				...
				if (intval($boloes[$i][0])==$solicitacao->bolao){
					...
					for ($j = 0; $j<count($boloes[$i][5]); $j++){
						if ($boloes[$i][5][$j]!=';'){
							...
							$apostadoresBolao = $apostadoresBolao . ',' . ($solicitacao->usuarioRemetente)->cpf . ';';
							$boloes[$i][5] = $apostadoresBolao;
							break;
						}
					}
			...
			(DataGetter::getInstance())->appendData('boloes_' . ($solicitacao->usuarioRemetente)->cpf, 'ativo;' . $solicitacao->bolao); //adiciona a boloes que participa
		}
		(DataGetter::getInstance())->setData('notificacoes_' . $this->cpf, $novasNot);
	}
	...
	*/
	function excluirApostadorBolao($idBolao, $cpfApostador) {
		...
		$boloesuser = (DataGetter::getInstance())->getData('boloes_' . $cpfApostador);
		$novosboloes = array();
	...
	*/
	function cadastrarResultados($placar, $jogo, $bolao) {
		...
		$jogos = (DataGetter::getInstance())->getData('jogos_' . $bolao);
